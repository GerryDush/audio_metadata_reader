...
Future<ParserTag> readAllMetadata(File track, {bool getImage = true}) async {
  final reader = FileRandomAccessFile(randomAccessFile: track.openSync());
...
Future<AudioMetadata> readMetadataWebDav(webdav.Client client,String path, {bool getImage = false}) async {
  ...
  try {
    ...
    Error.throwWithStackTrace(
        MetadataParserException(track: File(''), message: e.message), s);
  } catch (e, s) {
    Error.throwWithStackTrace(
        MetadataParserException(track: File(''), message: e.toString()), s);
  }
